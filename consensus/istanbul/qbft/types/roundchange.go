...
func (p *SignedRoundChangePayload) DecodeRLP(stream *rlp.Stream) error {
	...
	if _, err := stream.List(); err != nil {
		log.Error("QBFT: Error List() Signed Payload", "struct", "SignedRoundChangePayload", "err", err)
		return err
	}
...
func (p *SignedRoundChangePayload) encodePayloadInternal() ([]byte, error) {
	...
	if p.PreparedRound != nil && p.PreparedDigest != (common.Hash{}) {
...
func (m *RoundChange) DecodeRLP(stream *rlp.Stream) error {
	...
	// Signed Payload
	if _, err = stream.List(); err != nil {
		log.Error("QBFT: Error List() Signed Payload", "struct", "RoundChange", "err", err)
		return err
	}
